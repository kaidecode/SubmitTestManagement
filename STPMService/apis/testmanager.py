...
import json
from utils.emialUtils import sendEmail
...
# 表单提交相关校验
class fileForm(Form):
    file = FileField(validators=[FileRequired(), FileAllowed(['jpg', 'png', 'pdf', 'zip'])])
...
def searchBykey():
    ...
    with connection:
        ...
        with connection.cursor() as cursor:
            count_select = 'SELECT COUNT(*) as `count` FROM request as R , apps as A where R.appId = A.appId AND R.isDel=0' + sql
            print(count_select)
            cursor.execute(count_select)
            total = cursor.fetchall()

        # sql lim 那句注释掉放在这里：application中
        sql = sql + ' ORDER BY R.updateDate DESC LIMIT {},{}'.format((currentPage - 1) * pageSize, pageSize)
        # print(sql)
        # 执行查询
        with connection.cursor() as cursor:
            # 按照条件进行查询
            cursor.execute('SELECT A.appId,R.* FROM request as R , apps as A where R.appId = A.appId AND R.isDel=0' + sql)
        ...
        try:
            with connection.cursor() as cursor:
                ...
                cursor.execute(sqlInsert, (
                    body["title"], body["appName"], body["developer"], body["tester"], body["CcMail"], body["version"],
                    body['type'],
            ...
            resp_failed['message'] = '提测失败了:' + str(err)
            return resp_failed
...
def getTestInfo():
    ...
    with connection.cursor() as cursor:
        # 查询产品信息表-按更新时间新旧排序
        sql = "SELECT A.id as appId, A.appId as appName, R.id,R.title,R.developer,R.tester,R.CcMail,R.version,R.type,R.scope,R.gitCode,R.wiki,R.more FROM request as R , apps as A where R.appId = A.appId AND R.isDel=0 AND R.id={}".format(test_id)
        cursor.execute(sql)
        data = cursor.fetchall()
        if len(data) == 1:
...
def updateReqeust():
    ...
    with connection:
        with connection.cursor() as cursor:
            sql = "SELECT A.id as appId, A.appId as appName, R.id,R.title,R.developer,R.tester,R.CcMail,R.version,R.type,R.scope,R.gitCode,R.wiki,R.more FROM request as R , apps as A where R.appId = A.appId AND R.isDel=0 AND R.id={}".format(body['id'])
...
def uploadFile():
    ...
    print(file_form.validate())
    if file_form.validate():
        # 获取项目路径+保存文件夹，组成服务保存绝对路径
        save_path = os.path.join(os.path.abspath(os.path.dirname(__file__)).split('STPMService')[0], 'STPMService/static')
